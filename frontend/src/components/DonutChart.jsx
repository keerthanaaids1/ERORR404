...
export default function DonutChart({ data = [], darkMode = false }) {
  const chartData = data.map(item => ({ name: item.category, value: item.percentage, details: item.details }));

  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      return (
        <div className={`border rounded-2xl p-3 shadow-med transition-colors ${darkMode ? 'bg-[#18231C] border-[#24352B]' : 'bg-white border-[#E6E2DA]'}`}>
          <p className={`text-xs font-serif font-bold ${darkMode ? 'text-white' : 'text-[#2D3A31]'}`}>{payload[0].name}</p>
    ...
    <div className="flex flex-col md:flex-row items-center gap-6 w-full h-full">
      <div className="w-full md:w-1/2 h-[220px] relative">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            <Pie data={chartData} cx="50%" cy="50%" innerRadius={60} outerRadius={85}
              ...
              {chartData.map((_, i) => (
                <Cell key={i} fill={COLORS[i % COLORS.length]} stroke={darkMode ? '#18231C' : '#F9F8F4'} strokeWidth={3} />
              ))}
            </Pie>
          </PieChart>
        </ResponsiveContainer>
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
          ...
          <span className={`text-xl font-serif font-bold ${darkMode ? 'text-white' : 'text-[#2D3A31]'}`}>100%</span>
        </div>
      </div>

      <div className="w-full md:w-1/2 flex flex-col gap-3">
        {data.map((item, i) => (
          <div key={i} className={`flex items-start gap-3 pb-3 border-b ${darkMode ? 'border-[#24352B]' : 'border-[#E6E2DA]'} last:border-0`}>
            <div className="w-3 h-3 rounded-full mt-1 flex-shrink-0" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
            <div className="flex-1">
              <div className="flex justify-between items-center">
                <span className={`text-sm font-semibold font-sans ${darkMode ? 'text-white' : 'text-[#2D3A31]'}`}>{item.category}</span>

...
export default function Navbar() {
  ...
  return (
    <>
      <nav className="sticky top-0 z-50 bg-[#F9F8F4]/90 dark:bg-[#0F1712]/90 backdrop-blur-md border-b border-[#E6E2DA] dark:border-[#24352B] py-4 px-6 md:px-12 flex items-center justify-between transition-colors duration-300">
        <Link to="/" className="flex items-center gap-2 group">
          ...
          <span className="font-serif font-bold text-[#2D3A31] dark:text-white text-lg transition-colors">WealthPath <span className="text-[#8C9A84] italic">AI</span></span>
        </Link>

        <div className="hidden md:flex items-center gap-8 text-sm font-medium text-[#8C9A84]">
          <Link to="/" className="hover:text-[#2D3A31] dark:hover:text-white transition-colors duration-300">Home</Link>
          <a href="#how-it-works" className="hover:text-[#2D3A31] dark:hover:text-white transition-colors duration-300">How It Works</a>
          <a href="#features" className="hover:text-[#2D3A31] dark:hover:text-white transition-colors duration-300">Features</a>
        </div>
        ...
        <div className="flex items-center gap-3 md:gap-4">
          {user ? (
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-full bg-[#2D3A31] dark:bg-[#8C9A84] text-white dark:text-[#0F1712] flex items-center justify-center font-bold text-xs select-none transition-colors">
                  {user.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                </div>
                <span className="hidden sm:inline text-xs font-semibold text-[#2D3A31] dark:text-white transition-colors">{user.name}</span>
              </button>
              {menuOpen && (
                <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-[#18231C] border border-[#E6E2DA] dark:border-[#24352B] rounded-2xl shadow-large p-2 z-50 animate-[fadeIn_0.15s_ease-out]">
                  <div className="px-3 py-2 border-b border-[#E6E2DA] dark:border-[#24352B] mb-1">
                    <p className="text-xs font-bold text-[#2D3A31] dark:text-white truncate transition-colors">{user.name}</p>
                    <p className="text-[10px] text-[#8C9A84] truncate">{user.email}</p>
                  </div>
                  
                  {savedRoadmaps.length > 0 && (
                    <div className="px-3 py-1.5 border-b border-[#E6E2DA] dark:border-[#24352B] mb-1">
                        ...
                        {savedRoadmaps.map((item, idx) => {
                          ...
                          return (
                            <button
                              ...
                              className="w-full text-left text-[11px] text-[#2D3A31] dark:text-[#F0F5F2] hover:bg-[#F2F0EB] dark:hover:bg-[#121C16] p-1.5 rounded transition-colors truncate font-sans"
                  ...
                  <button 
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-[#2D3A31] dark:text-white hover:bg-[#F2F0EB] dark:hover:bg-[#121C16] rounded-lg transition-colors flex items-center gap-2"
                  >
            ...
            <div className="flex items-center gap-3">
              <button 
                ...
                className="text-xs text-[#2D3A31] dark:text-white font-semibold hover:text-[#8C9A84] px-4 py-2 transition-all duration-300"
              >
                Log In
              </button>
              <button 
          ...
          <button onClick={toggleTheme}
            className="flex items-center justify-center w-9 h-9 border border-[#E6E2DA] dark:border-[#24352B] rounded-full transition-all shadow-soft bg-white dark:bg-[#18231C] text-[#2D3A31] dark:text-[#F0F5F2] hover:border-[#8C9A84] select-none cursor-pointer"
            title={darkMode ? "Switch to Light Mode" : "Switch to Dark Mode"}>
            {darkMode ? '☀️' : '🌙'}
          </button>

          <button onClick={() => setOpen(!open)} className="md:hidden text-[#2D3A31] dark:text-white p-1">
            {open ? <X className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
        </div>

        {open && (
          <div className="fixed inset-0 bg-[#F9F8F4]/98 dark:bg-[#0F1712]/98 backdrop-blur-xl z-40 flex flex-col justify-between pt-28 pb-12 px-8 md:hidden">
            <div className="flex flex-col gap-6 text-2xl font-serif font-bold text-[#2D3A31] dark:text-white">
              <Link to="/" onClick={() => setOpen(false)} className="border-b border-[#E6E2DA] dark:border-[#24352B] pb-4">Home</Link>
              <a href="#how-it-works" onClick={() => setOpen(false)} className="border-b border-[#E6E2DA] dark:border-[#24352B] pb-4">How It Works</a>
              <a href="#features" onClick={() => setOpen(false)} className="border-b border-[#E6E2DA] dark:border-[#24352B] pb-4">Features</a>
              {!user && (
                <button 
                  onClick={() => { setOpen(false); setIsSignUp(false); setShowAuthModal(true); }}
                  className="text-left border-b border-[#E6E2DA] dark:border-[#24352B] pb-4 text-[#2D3A31] dark:text-white"
                >
                  Sign In
                </button>
              )}
            ...
            <div className="flex flex-col gap-4">
              {user && (
                <div className="flex items-center gap-3 p-4 bg-[#F2F0EB] dark:bg-[#121C16] rounded-2xl mb-2">
                  <div className="w-10 h-10 rounded-full bg-[#2D3A31] dark:bg-[#8C9A84] text-white dark:text-[#0F1712] flex items-center justify-center font-bold text-sm">
                    {user.name.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase()}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-[#2D3A31] dark:text-white">{user.name}</p>
      ...
      {showAuthModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-[#18231C] border border-[#E6E2DA] dark:border-[#24352B] rounded-3xl p-8 max-w-sm w-full max-h-[90vh] overflow-y-auto shadow-large z-[110] animate-[fadeIn_0.25s_ease-out]">
            <button 
              onClick={() => setShowAuthModal(false)}
              disabled={loading}
              className="absolute top-4 right-4 text-[#8C9A84] hover:text-[#2D3A31] dark:hover:text-white p-1 disabled:opacity-50 transition-colors"
            >
              <X className="w-5 h-5" />
            </button>
            ...
            <div className="text-center mb-6">
              ...
              <h3 className="font-serif text-2xl font-bold text-[#2D3A31] dark:text-white mb-1">
              ...
              <div className="flex flex-col gap-4">
                  ...
                  {isSignUp && (
                    <div className="flex flex-col gap-1">
                      ...
                      <input 
                        ...
                        className="w-full bg-[#F2F0EB] dark:bg-[#121C16] rounded-xl px-4 py-2.5 text-xs text-[#2D3A31] dark:text-white focus:outline-none focus:border-[#8C9A84] border border-transparent transition-colors"
                      />
                    </div>
                  )}
                ...
                <div className="text-center mt-2">
                  <button
                    ...
                    className="text-[11px] text-[#8C9A84] hover:text-[#2D3A31] dark:hover:text-white underline font-medium"
                  >

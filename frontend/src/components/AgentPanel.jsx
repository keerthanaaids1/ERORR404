        ...
        return (
          <div key={agent.id}
            className={`flex items-start gap-5 p-5 rounded-3xl border transition-all duration-500 ${
              state === 'ACTIVE'
                ? 'bg-white border-[#8C9A84] shadow-med translate-x-1'
                : state === 'DONE'
                ? 'bg-[#F2F0EB] border-[#8C9A84]/40'
                : 'bg-white/50 border-[#E6E2DA] opacity-40'
            }`}>
            <div className="relative flex-shrink-0">
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center border transition-all duration-300 ${
                ...
                : state === 'DONE'  ? 'bg-[#2D3A31]/10 border-[#2D3A31] text-[#2D3A31]'
                : 'bg-[#E6E2DA] border-[#E6E2DA] text-[#8C9A84]'
              }`}>
                <Icon className="w-5 h-5" strokeWidth={1.5} />
              </div>
              {state === 'DONE' && (
                <div className="absolute -bottom-1.5 -right-1.5 w-5 h-5 rounded-full bg-[#2D3A31] flex items-center justify-center">
            ...
            <div className="flex-grow">
              <div className="flex items-center gap-2 flex-wrap">
                <h4 className={`font-serif font-bold text-sm ${state === 'WAITING' ? 'text-[#8C9A84]' : 'text-[#2D3A31]'}`}>
                  {agent.name}
                </h4>
                <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium border uppercase tracking-wider ${
                  ...
                  : state === 'DONE' ? 'bg-[#2D3A31]/10 border-[#2D3A31]/20 text-[#2D3A31]'
                  : 'bg-[#E6E2DA] border-[#E6E2DA] text-[#8C9A84]'
                }`}>{agent.role}</span>
              </div>
              <p className="text-xs text-[#8C9A84] mt-1 leading-relaxed font-sans">{agent.desc}</p>
              {state === 'ACTIVE' && (
                <div className="mt-3 w-full bg-[#E6E2DA] rounded-full h-1 overflow-hidden">
